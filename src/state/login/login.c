...
Login_User(CLIENT * Client)
{
	...
	/* Keep CAP negotiation non-blocking for the Windows client path.
	 * Clients that want CAP still receive the CAP replies, but basic
	 * registration must not stall if CAP END is delayed or omitted. */
	if (Client_Cap(Client) & CLIENT_CAP_PENDING) {
		LogDebug("Connection %d: CAP pending during login, continuing registration.",
			 conn);
