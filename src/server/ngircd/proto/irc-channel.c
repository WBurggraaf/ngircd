	     ...
	     const char *key)
{
	...
	JOIN_CHECK_RESULT check;
	bool is_operator, is_registered, has_ssl, key_ok, is_full;

	is_operator = Client_HasMode(Client, 'o');
	...
	key_ok = Channel_CheckKey(chan, Client, key ? key : "");
	is_full = Channel_HasMode(chan, 'l') &&
		(Channel_MaxUsers(chan) <= Channel_MemberCount(chan));
	has_ssl = Conn_UsesSSL(Client_Conn(Client));
	is_registered = Client_HasMode(Client, 'R');

	check = Join_Check_Access(is_operator, is_banned, is_invited,
				  is_exception, key_ok, is_full,
				  Channel_HasMode(chan, 'z'), has_ssl,
				  Channel_HasMode(chan, 'O'),
				  Channel_HasMode(chan, 'R'),
				  is_registered);

	switch (Join_Error_FromCheck(check)) {
	case JOIN_ERROR_BANNED:
		/* Client is banned from channel (and not on invite list) */
		IRC_WriteErrClient(Client, ERR_BANNEDFROMCHAN_MSG,
				   Client_ID(Client), channame);
		return false;
	case JOIN_ERROR_INVITEONLY:
		/* Channel is "invite-only" and client is not on invite list */
		IRC_WriteErrClient(Client, ERR_INVITEONLYCHAN_MSG,
				   Client_ID(Client), channame);
		return false;
	case JOIN_ERROR_BADKEY:
...
IRC_JOIN( CLIENT *Client, REQUEST *Req )
{
	...
	if (Message_Check_Join(Client, Req) != MESSAGE_CHECK_OK)
