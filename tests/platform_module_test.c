/*
 * platform_module_test.c — unit tests for the platform wrapper DLL API
 *
 * Compiles src/platform/platform.c directly with PLATFORM_BUILD.
 * No external function stubs required (platform.c only calls POSIX/CRT).
 *
 * Tests:
 ...
 *   - open_file("")           error path returns IO_ERROR
 */
...
/* Forward-declare module_get_api_v1 compiled from platform.c */
...
int main(void)
{
    ...
    /* ---- Error-path: open empty path → IO_ERROR ---- */
    CHECK(api->open_file("", 0) == CORE_STATUS_IO_ERROR, "platform_open_empty_path");
