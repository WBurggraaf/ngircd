...
Conn_FromSocket(int Sock)
{
	...
	if (Sock <= NONE || My_Connections == NULL || Pool_Size <= 0)
		return NULL;

	for (i = 0; i < Pool_Size; ++i) {
		if (My_Connections[i].sock == Sock)
			return &My_Connections[i];
	}
...
static void
Log_BufferPreview(const char *label, CONN_ID Idx, const char *buf, size_t len)
{
	size_t i, preview = len < 32 ? len : 32;
	char hex[32 * 3 + 1];
	char *p = hex;

	if (!buf) {
		LogDebug("%s(idx=%d): buffer is NULL (len=%lu).",
			 label, Idx, (unsigned long)len);
		return;
	}

	for (i = 0; i < preview; ++i) {
		p += snprintf(p, (size_t)(hex + sizeof(hex) - p), "%02X%s",
			      (unsigned char)buf[i], (i + 1 < preview) ? " " : "");
	}
	*p = '\0';
	LogDebug("%s(idx=%d): len=%lu preview=%lu bytes hex=[%s]%s",
		 label, Idx, (unsigned long)len, (unsigned long)preview, hex,
		 (len > preview) ? " ..." : "");
}
...
{
	(void) irrelevant;
	LogDebug("cb_listen(sock=%d): readable, accepting ...", sock);
...
{
	CONNECTION *conn = Conn_FromSocket(sock);
	CONN_ID idx = (conn != NULL) ? (CONN_ID)CONNECTION2ID(conn) : NONE;

	LogDebug("cb_clientserver(sock=%d, what=%d, idx=%d)", sock, what, idx);
	if (conn == NULL) {
		Log(LOG_ERR, "cb_clientserver(sock=%d): no matching connection found.", sock);
		io_close(sock);
		return;
	}
...
#ifdef SSL_SUPPORT
	if (what & IO_WANTREAD
	    || (Conn_OPTION_ISSET(conn, CONN_SSL_WANT_WRITE))) {
...
NewListener(const char *listen_addr, UINT16 Port)
{
	...
	LogDebug("NewListener(): socket=%d af=%d listen_addr=%s port=%u",
	         sock, af, listen_addr ? listen_addr : "(null)",
	         (unsigned int)Port);
...
Conn_Handler(void)
{
	...
	LogDebug("Conn_Handler(): entering main loop (Pool_Size=%d).", Pool_Size);
...
Handle_Write( CONN_ID Idx )
{
	...
	assert( Idx > NONE );
	LogDebug("Handle_Write(idx=%d): enter.", Idx);
	if (My_Connections == NULL || Pool_Size <= 0 || Idx >= Pool_Size) {
		Log(LOG_ERR, "Handle_Write(idx=%d): invalid pool state (base=%p pool=%d).",
		    Idx, (void *)My_Connections, Pool_Size);
		return false;
	}
	conn = &My_Connections[Idx];
	LogDebug("Handle_Write(idx=%d): base=%p conn=%p pool=%d",
		 Idx, (void *)My_Connections, (void *)conn, Pool_Size);
	if (conn->sock != Idx) {
		Log(LOG_ERR,
		    "Handle_Write(idx=%d): socket mismatch (slot sock=%d).",
		    Idx, conn->sock);
		return false;
	}
	if ( conn->sock < 0 ) {
	...
	LogDebug("Handle_Write(idx=%d): sock=%d wbuf.used=%lu wbuf.mem=%p",
		 Idx, conn->sock,
		 (unsigned long)conn->wbuf.used,
		 (void *)conn->wbuf.mem);
	if (conn->wbuf.used > 0)
		Log_BufferPreview("Handle_Write(): outbound buffer", Idx,
				  (const char *)array_start(&conn->wbuf),
				  conn->wbuf.used);
	wdatalen = array_bytes(&conn->wbuf);
	LogDebug("Handle_Write(idx=%d): wdatalen=%lu after array_bytes().",
		 Idx, (unsigned long)wdatalen);
	...
	if (wdatalen == 0) {
		...
		wdatalen = array_bytes(&conn->wbuf);
		LogDebug("Handle_Write(idx=%d): wdatalen=%lu after Zip_Flush().",
			 Idx, (unsigned long)wdatalen);
	}
#endif
	...
	if (wdatalen == 0) {
		/* Still no data, fine. */
		LogDebug("Handle_Write(idx=%d): no pending data.", Idx);
		LogDebug("Handle_Write(idx=%d): dropping write event for sock=%d.",
			 Idx, conn->sock);
		io_event_del(conn->sock, IO_WANTWRITE );
		return true;
	}
...
#ifdef SSL_SUPPORT
	if ( Conn_OPTION_ISSET( conn, CONN_SSL )) {
		LogDebug("Handle_Write(idx=%d): writing %lu bytes via SSL.",
			 Idx, (unsigned long)wdatalen);
		len = ConnSSL_Write(conn,
				    array_start(&conn->wbuf),
				    wdatalen);
	} else
#endif
	{
		LogDebug("Handle_Write(idx=%d): writing %lu bytes to sock=%d.",
			 Idx, (unsigned long)wdatalen, conn->sock);
#ifdef _WIN32
		len = send(conn->sock,
			   array_start(&conn->wbuf), (int)wdatalen, 0);
#else
		len = write(conn->sock,
			    array_start(&conn->wbuf), wdatalen);
#endif
	}
	LogDebug("Handle_Write(idx=%d): write/send returned %ld.",
		 Idx, (long)len);
	if( len < 0 ) {
		LogDebug("Handle_Write(idx=%d): write returned %ld errno=%d (%s).",
			 Idx, (long)len, errno, strerror(errno));
	...
	if ((size_t)len > wdatalen) {
		Log(LOG_ERR,
		    "Handle_Write(idx=%d): wrote more bytes than pending (%ld > %lu).",
		    Idx, (long)len, (unsigned long)wdatalen);
		return false;
	}

	/* move any data not yet written to beginning */
	LogDebug("Handle_Write(idx=%d): moving buffer left by %ld bytes.",
		 Idx, (long)len);
	array_moveleft(&conn->wbuf, 1, (size_t)len);
	LogDebug("Handle_Write(idx=%d): wbuf.used=%lu after moveleft.",
		 Idx, (unsigned long)conn->wbuf.used);
	LogDebug("Handle_Write(idx=%d): wrote %ld bytes.", Idx, (long)len);
...
New_Connection(int Sock, UNUSED bool IsSSL)
{
	...
	LogDebug("New_Connection(): accepting on listen socket=%d", Sock);
	...
	LogDebug("New_Connection(): idx=%d sock=%d peer=%s:%u listen_sock=%d",
	         new_sock, My_Connections[new_sock].sock,
	         My_Connections[new_sock].host,
	         (unsigned int)ng_ipaddr_getport(&new_addr),
	         Sock);
	Account_Connection();
...
Conn_StartLogin(CONN_ID Idx)
{
	...
	assert(Idx >= 0);
	LogDebug("Conn_StartLogin(idx=%d): dns=%d ident=%d notice=%d",
	         Idx, Conf_DNS, Conf_Ident, Conf_NoticeBeforeRegistration);
	...
	if (Conf_NoticeBeforeRegistration) {
		...
		if (Conf_Ident) {
			if (Conf_DNS) {
				LogDebug("Conn_StartLogin(idx=%d): writing hostname+ident notice", Idx);
				if (!Conn_WriteStr(Idx,
					"NOTICE * :*** Looking up your hostname and checking ident")) {
					Log(LOG_ERR, "Conn_StartLogin(idx=%d): failed to queue hostname+ident notice.", Idx);
					return;
				}
			} else {
				LogDebug("Conn_StartLogin(idx=%d): writing ident notice", Idx);
				if (!Conn_WriteStr(Idx,
					"NOTICE * :*** Checking ident")) {
					Log(LOG_ERR, "Conn_StartLogin(idx=%d): failed to queue ident notice.", Idx);
					return;
				}
			}
		} else
		...
		if(Conf_DNS)
		{
			LogDebug("Conn_StartLogin(idx=%d): writing hostname notice", Idx);
			if (!Conn_WriteStr(Idx,
				"NOTICE * :*** Looking up your hostname")) {
				Log(LOG_ERR, "Conn_StartLogin(idx=%d): failed to queue hostname notice.", Idx);
				return;
			}
		}
		else
		{
			LogDebug("Conn_StartLogin(idx=%d): writing processing notice", Idx);
			if (!Conn_WriteStr(Idx,
				"NOTICE * :*** Processing your connection")) {
				Log(LOG_ERR, "Conn_StartLogin(idx=%d): failed to queue processing notice.", Idx);
				return;
			}
		}
		 ...
		 * again in this case! */
		LogDebug("Conn_StartLogin(idx=%d): flushing notice buffer.", Idx);
		if (!Handle_Write(Idx))
		{
			Log(LOG_ERR, "Conn_StartLogin(idx=%d): notice flush failed.", Idx);
			return;
		}
		LogDebug("Conn_StartLogin(idx=%d): notice buffer flushed.", Idx);
	}

	LogDebug("Conn_StartLogin(idx=%d): starting resolver.", Idx);
...
Read_Request(CONN_ID Idx)
{
	...
	assert(Idx > NONE);
	if (My_Connections == NULL || Pool_Size <= 0 || Idx >= Pool_Size) {
		Log(LOG_ERR, "Read_Request(idx=%d): invalid pool state.", Idx);
		return;
	}
	if (My_Connections[Idx].sock != Idx) {
		Log(LOG_ERR, "Read_Request(idx=%d): socket mismatch (slot sock=%d).",
		    Idx, My_Connections[Idx].sock);
		return;
	}
	assert(My_Connections[Idx].sock > NONE);
	LogDebug("Read_Request(idx=%d sock=%d host=%s)", Idx, My_Connections[Idx].sock,
...
#endif
	{
		Log_BufferPreview("Read_Request(): append to rbuf", Idx, readbuf, (size_t)len);
	...
	LogDebug("Read_Request(idx=%d): read %ld bytes.", Idx, (long)len);
	LogDebug("Read_Request(idx=%d): rbuf.used=%lu bytes_in=%ld bytes_out=%ld.",
		 Idx, (unsigned long)array_bytes(&My_Connections[Idx].rbuf),
		 My_Connections[Idx].bytes_in, My_Connections[Idx].bytes_out);
