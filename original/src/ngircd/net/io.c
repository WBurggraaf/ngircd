...
{
#ifdef _WIN32
	return (fd >= 0) && ((SOCKET)fd != INVALID_SOCKET);
#else
	return fd >= 0;
#endif
}
...
io_close_select(int fd)
{
	...
	FD_CLR((unsigned int)fd, &writers);
	FD_CLR((unsigned int)fd, &readers);
...
{
	const NgIoOps *ops = NgIo_GetOps();

	if (ops && ops->library_init)
		return ops->library_init(eventsize);
...
io_library_shutdown(void)
{
	...
	if (ops && ops->library_shutdown) {
		ops->library_shutdown();
		return;
	}
...
io_event_create(int fd, short what, void (*cbfunc) (int, short))
{
	...
	assert(fd >= 0);
	if (!io_fd_valid(fd)) {
		Log(LOG_ERR, "io_event_create(): invalid fd/socket %d.", fd);
		return false;
	}
...
io_event_add(int fd, short what)
{
	...
	if (!i) return false;
	if (!io_fd_valid(fd)) {
		Log(LOG_ERR, "io_event_add(): invalid fd/socket %d for what=%d.", fd, what);
		return false;
	}
	...
	if (what & IO_WANTREAD)
		FD_SET((unsigned int)fd, &readers);
	if (what & IO_WANTWRITE)
		FD_SET((unsigned int)fd, &writers);
...
io_docallback(int fd, short what)
{
	...
	if (i->callback) {	/* callback might be NULL if a previous callback function
		...
		LogDebug("io_docallback(fd=%d, what=%d): callback=%p registered=%d.",
			 fd, what, (void *)i->callback, (int)i->what);
