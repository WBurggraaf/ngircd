...
IRC_PASS( CLIENT *Client, REQUEST *Req )
{
	...
	switch (Login_Check_Pass(Client_Type(Client), Req->argc)) {
	case LOGIN_CHECK_ALLOWED:
		break;
	case LOGIN_CHECK_NEEDMOREPARAMS:
		return IRC_WriteErrClient(Client, ERR_NEEDMOREPARAMS_MSG,
					  Client_ID(Client), Req->command);
	case LOGIN_CHECK_ALREADYREGISTERED:
		return IRC_WriteErrClient(Client, ERR_ALREADYREGISTRED_MSG,
					  Client_ID(Client));
	case LOGIN_CHECK_UNKNOWNCOMMAND:
	default:
...
IRC_NICK( CLIENT *Client, REQUEST *Req )
{
	...
	switch (Nick_Check_Context(Client_Type(Client), Req->argc)) {
	case NICK_CHECK_USER_OR_CHANGE:
		...
		if (Client_Type(target) != CLIENT_USER &&
			...
			switch (Login_Flow_OnNick(Client_Type(Client),
						  Client_Type(Client) == CLIENT_GOTUSER,
						  Client_Cap(Client) & CLIENT_CAP_PENDING,
#ifndef STRICT_RFC
						  Conn_GetAuthPing(Client_Conn(Client)) != 0,
#else
						  false,
#endif
						  true))
			{
			case LOGIN_FLOW_LOGIN_NOW:
				return Login_User( Client );
			case LOGIN_FLOW_SET_GOTNICK:
		...
		return CONNECTED;
	case NICK_CHECK_SERVER_INTRODUCE:
		...
		return CONNECTED;
	case NICK_CHECK_ALREADY_REGISTERED:
	default:
		return IRC_WriteErrClient(Client, ERR_ALREADYREGISTRED_MSG,
					  Client_ID(Client));
	}
...
IRC_USER(CLIENT * Client, REQUEST * Req)
{
	...
	switch (User_Check_Context(Client_Type(Client))) {
	case USER_CHECK_NEW_CONNECTION:
		/* New connection */
		...
		LogDebug("Connection %d: got valid USER command ...",
		    Client_Conn(Client));
		switch (Login_Flow_OnUser(Client_Type(Client),
	...
	case USER_CHECK_SERVER_UPDATE:
		...
		return CONNECTED;
	case USER_CHECK_ALREADY_REGISTERED:
		/* Already registered connection */
		return IRC_WriteErrClient(Client, ERR_ALREADYREGISTRED_MSG,
					  Client_ID(Client));
	case USER_CHECK_NOT_REGISTERED:
	default:
