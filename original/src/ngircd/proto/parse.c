...
Parse_Request( CONN_ID Idx, char *Request )
{
	...
	LogDebug("Parse_Request(idx=%d): raw='%s'", Idx, Request);
	LogDebug("Parse_Request(idx=%d): raw_len=%lu", Idx, (unsigned long)strlen(Request));
	...
	if (Request[0] == '\0') {
		LogDebug("Parse_Request(idx=%d): empty command line ignored.", Idx);
		return true;
	}
	...
	LogDebug("Parse_Request(idx=%d): cmd=%s argc=%d prefix=%s", Idx,
	         req.command ? req.command : "(null)", req.argc,
	         req.prefix ? req.prefix : "(none)");
	for (int argi = 0; argi < req.argc; ++argi) {
		LogDebug("Parse_Request(idx=%d): argv[%d]='%s'", Idx, argi,
		         req.argv[argi] ? req.argv[argi] : "(null)");
	}
...
Validate_Prefix( CONN_ID Idx, REQUEST *Req, bool *Closed )
{
	...
	*Closed = false;
	...
	assert( client != NULL );
	LogDebug("Handle_Request(idx=%d): client_type=%d command=%s argc=%d prefix=%s",
	         Idx, Client_Type(client), Req->command, Req->argc,
	         Req->prefix ? Req->prefix : "(none)");
	LogDebug("Handle_Request(idx=%d): cmd=%s client_type=%d argc=%d", Idx,
	         Req->command ? Req->command : "(null)", Client_Type(client), Req->argc);
...
Handle_Request( CONN_ID Idx, REQUEST *Req )
{
	...
	while (cmd->name) {
		...
		Conn_ResetWCounter();
		LogDebug("Handle_Request(idx=%d): dispatching %s handler=%p",
			 Idx, cmd->name, (void *)cmd->function);
		LogDebug("Dispatch(idx=%d): %s -> %p", Idx, cmd->name, (void*)cmd->function);
	...
	LogDebug("Connection %d: Unknown command \"%s\", %d %s,%s prefix.",
			...
			Req->prefix ? "" : " no" );
	LogDebug("Handle_Request(idx=%d): unknown command path taken.", Idx);

	/* Unknown command: generate error for local clients; server links
	 * handle unknown commands through network-level routing. */
	if (client_type != CLIENT_SERVER)
